...
PirReply PIRServer::generate_reply(PirQuery query, uint32_t client_id) {
    auto expanded_queries = expand_query(query, client_id);
    return generate_reply_with_expanded_queries(expanded_queries, client_id);
}

    
PirReply PIRServer::generate_reply_with_expanded_queries(std::vector<std::vector<seal::Ciphertext>> expanded_queries, std::uint32_t client_id) {
    ...
    for (uint32_t i = 0; i < nvec.size(); i++) {
        ...
        auto expanded_query = expanded_queries[i];
...
vector<std::vector<seal::Ciphertext>> PIRServer::expand_query(PirQuery query, uint32_t client_id) {
    ...
    auto expanded_queries = vector<vector<Ciphertext>>();
    ...
    for (uint32_t i = 0; i < nvec.size(); i++) {
        ...
        */
        ...
        expanded_queries.push_back(expanded_query);
    }
